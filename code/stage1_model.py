...
class EqualLinear(nn.Module):
    def __init__(self, in_dim, out_dim, bias=True, bias_init=0, lr_mul=1, activation=None, custom_activation=None):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, size, dim=512, dim_motion=20):
        ...
        fc.append(EqualLinear(dim, dim_motion, activation=True, custom_activation=TanH()))
    ...
    def encode(self, x):
        h_source = self.net_app(x)
        h_motion = self.fc(h_source[0])
        return h_source, h_motion
...
class ToFlow(nn.Module):
    def __init__(self, in_channel, style_dim, upsample=True, blur_kernel=[1, 3, 3, 1]):
        ...
        self.conv_gen = ModulatedConv2d(in_channel, in_channel, 1, style_dim, demodulate=False)
        self.bias = nn.Parameter(torch.zeros(1, 3, 1, 1))

    def forward(self, input, style, feat, skip=None):
        out = self.conv(input, style)
        out2 = self.conv_gen(input, style)
        out = out + self.bias
        ...
        return feat_warp, feat_warp + out2 * (1.0 - mask), out
...
class RefineBlock(nn.Module):
    def __init__(self, in_channel=3, out_channel=3, hidden_channel=512, depth=3):
        super(RefineBlock, self).__init__()
        self.convs = nn.ModuleList()
        for _ in range(depth):
            self.convs.append(nn.Conv2d(in_channel, hidden_channel, kernel_size=3, padding=1))
            self.convs.append(nn.ReLU(inplace=True))
            in_channel = hidden_channel
        self.final_conv = nn.Conv2d(hidden_channel, out_channel, kernel_size=3, padding=1)
...
class Synthesis(nn.Module):
    def __init__(self, size, style_dim, motion_dim, blur_kernel=[1, 3, 3, 1], channel_multiplier=1, refine_block=True):
    ...
    def forward(self, wa, alpha, feats):
        ...
        if self.refine_block:
            img = self.refine_block(img)
        return img


class Generator(nn.Module):
    def __init__(self, size, style_dim=512, motion_dim=20, channel_multiplier=1, blur_kernel=[1, 3, 3, 1], refine_block=True):
        super(Generator, self).__init__()

        # encoder
        self.enc = Encoder(size, style_dim, motion_dim)
        self.dec = Synthesis(size, style_dim, motion_dim, blur_kernel, channel_multiplier, refine_block=refine_block)
